...
class Rule_(BinaryOperator):
    ...
    # TODO: An error message should appear when Rule is called with a wrong number of arguments
    """
    >> a /. Rule[1, 2, 3] -> t
    ...
    if any_lists:
        ...
        for rule in rules:
            if rule.get_head_name() not in ('Rule', 'RuleDelayed'):
                ...
                evaluation.message(rule.get_head_name(), 'argrx', rule.get_head_name(), 3, 2)
                return None, True
            else:
...
class ReplaceList(Builtin):
    ...
    def apply(self, expr, rules, max, evaluation):
        ...
        rules, ret = create_rules(rules, expr, 'ReplaceList', evaluation, extra_args=[max])
        if ret:
            return rules
...
class Pattern_(PatternObject):
    ...
    rules = {
        'MakeBoxes[Verbatim[Pattern][symbol_Symbol, blank_Blank|blank_BlankSequence|blank_BlankNullSequence], f:StandardForm|TraditionalForm|InputForm|OutputForm]':
            'MakeBoxes[symbol, f] <> MakeBoxes[blank, f]', #'StringForm["`1``2`", HoldForm[symbol], blank]', 
    }
        ...
        if expression.has_form('Sequence', 0):
            if self.default is None:
                if head is None: # head should be given by match_leaf!
                    ...
                    name = head.get_name()
                    default = get_default_value(name, evaluation, leaf_index, leaf_count)
                if default is None:
...
class Condition(BinaryOperator, PatternObject):
    ...
    operator = '/;'
    precedence = 130
    attributes = ('HoldRest',)  # don't know why this has attribute HoldAll in Mathematica
