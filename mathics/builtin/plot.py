...
class _Plot(Builtin):
    ...
    def apply(self, functions, x, start, stop, evaluation, options):
        ...
        # unfortunately, most browsers seem to have problems with scaling stroke with properly. since we scale a
        # very tiny area, we specify a very small stroke width (e.g. 1e-6) which is then scaled. but most browsers
        # simply round this stroke width to 0 before scaling to we end up with an empty plot. in order to fix this,
        # PrecomputeTransformations simply gets rid of the SVG transformations and passes the scaled coordinates
        # into the SVG. this also has the advantage that we can precompute with arbitrary precision using mpmath.
        options['System`PrecomputeTransformations'] = Symbol('True')
