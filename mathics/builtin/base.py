    ...
    def __new__(cls, *args, **kwargs):
        if kwargs.get('expression', None) != False:
            ...
            if not instance.formats:
                # Reset formats so that not every instance shares the same empty dict {}
                instance.formats = {}
            return instance
        ...
        if self.default_formats:
            ...
            if not op_pattern in self.formats:
                ...
                if operator is not None:
                    self.formats[op_pattern] = '%s[{HoldForm[item]},"%s",%d]' % (format_function, operator, self.precedence)
...
class PrefixOperator(UnaryOperator):  
    ...
    def parse(self, args):
        ...
        if rest:           
            ...
            items = rest + [Token('(')] + [args[1]] + args[2].parse_tokens + [Token(')')]
...
class PostfixOperator(UnaryOperator):
    ...
    def parse(self, args):
        ...
        if rest:           
            ...
            items = [Token('(')] + args[0].parse_tokens + [args[1]] + [Token(')')] + rest
            result = parser.parse(items)
            return result
        else:
...
class BinaryOperator(Operator):
    ...
    def __init__(self, *args, **kwargs):
        ...
        name = self.get_name()
        # Prevent pattern matching symbols from gaining meaning here using Verbatim
        name = 'Verbatim[%s]' % name
        ...
        if self.default_formats: 
            operator = self.get_operator_display()
            formatted = 'MakeBoxes[Infix[{%s},"%s",%d,%s], form]' % (replace_items, operator, self.precedence, self.grouping)
            formatted_output = 'MakeBoxes[Infix[{%s}," %s ",%d,%s], form]' % (replace_items, operator, self.precedence, self.grouping)
            default_rules = {
        ...
        if grouping != 'NonAssociative':
            ...
            if grouping == 'None':
                ...
                for leaf in reversed(leaves[:-2]):
                    result = Expression(name, leaf, result, parse_operator=self)
                return result
            elif grouping == 'Left':
                result = Expression(name, parse_operator=self, *leaves[:2])
                for leaf in leaves[2:]:
                    result = Expression(name, result, leaf, parse_operator=self)
                return result
        else:        
