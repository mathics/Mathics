...
logical_algebraic_rules_spec = {
    ...
    "Or[pred1___, a_==b_, pred2___ , b_==a_, pred3___]":
    "Or[pred1, a==b, pred2, pred3]",
    "And[pred1___, a_==b_, pred2___ , b_==a_, pred3___]":
    "And[pred1, a==b, pred2, pred3]",
    "Or[pred1___, a_==b_, pred2___ , Not[b_==a_], pred3___]":
    "Or[pred1, pred2, pred3]",
    "And[pred1___, a_==b_, pred2___ , Not[b_==a_], pred3___]":
    "False",    
    "Xor[q_,  Not[q_]]": "True",
...
def logical_expand_assumptions(assumptions_list, evaluation):
    ...
    if changed:
        new_assumptions_list = remove_duplicated_assumptions(new_assumptions_list, evaluation)
...
def algebraic_expand_assumptions(assumptions_list, evaluation):
    ...
    for assumption in assumptions_list:
        if assumption.has_form("Not", 1):
            nas, local_change = algebraic_expand_assumptions([assumption._leaves[0]], evaluation)
            if local_change:
    ...
    if changed:
        assumptions_list = remove_duplicated_assumptions(new_assumptions_list, evaluation)
        new_assumptions_list = []
        for assumption in assumptions_list:
            assumption, applied = assumption.apply_rules(
                logical_algebraic_rules, evaluation
    ...
    for pat in assumptions_list:
        ...
        if pat.has_form("Equal", 2):
            ...
            if value:
                ...
                assumption_rules.append(Rule(Expression(pat._head, pat._leaves[1], pat._leaves[0]), SymbolFalse))
                for head in ("Equal", "Equivalent"):
                    assumption_rules.append(Rule(Expression(head, pat._leaves[0], pat._leaves[1]), SymbolFalse))
                    assumption_rules.append(Rule(Expression(head, pat._leaves[1], pat._leaves[0]), SymbolFalse))
            else:
                assumption_rules.append(Rule(pat, SymbolFalse))
        else:
            assumption_rules.append(Rule(pat, symbol_value))
