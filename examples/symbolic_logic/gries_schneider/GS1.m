...
(* ***************************************************************************
    ...
    GS0.m, which we include in the statement below, contains tooling so we can
    write 'expected' and 'actual' in our examples. You don't need to understand
    how this works. You just need to know how to use it, and you'll see how in
...
(* ***************************************************************************
    ...
    You don't have to write stars for commutative multiplication in mathics: all
    the following are the same: 2 * y, y * 2, 2 y, 2y. Notice that even in the
    PRINTING of the expressions, the stars are removed. Despite
    "HoldAllComplete" and "HoldForm", mathics always does a little
    simplification. You DO need to leave spaces between alphabetic symbols,
    however; 4a c is 4*a*c, whereas 4ac is 4*ac, where ac is a symbol with two
    characters.
...
(* Top of page 10: ************************************************************
    ...
    The expected output illustrates that mathics applies a list of rules in
    parallel. If mathics applied them in sequence, the first example would
    produce x + 2x or 3x, as in the second example, where we force the rules to
    apply in sequence, one after the other.
...
(* Textual Substitution and Hidden Variables, page 10 *************************
    ...
    physics. Author's manuals for both the American Physical Society and the
    American Mathematical Society declare that 1/2a means 1/(2a). This serious
    problem leads to subtle bugs!.
...
(* In the above, we made assignments to Q and x. We modified the global state of
   this session. We must clear out those assignment so that we can use the
   variables Q and x below without having to know or remember that we gave them
   values above. Clearing symbols before and after using them is a safety move
   and we frequently use it. *)
...
expect[ y,   y /. {x -> 2} ] (* ditto, "x" does not appear in "y" *)
...
(* Here's one broken over two lines because it's long. It has an additional
   twist intended to invite you to think a little. The rewrite rule "x -> x + y"
   replaces "x" with ... something that has "x" in it. There is only one way for
   this to make sense without going into an "infinite loop," but you should
   puzzle it out and not stop until you're convinced you understand it. *)

expect[ (x + y)^2 + y^2 + (x + y)^3,

        (x^2 + y^2 + x^3)                  /. {x -> x + y} ]

(* Here are some more adapted from the book. *)
...
|___/\_,_|_.__/__/\__|_|\__|\_,_|\__|_\___/_||_|
    ...
    take some liberties with G&S and write adaptations of their formulas with
    small (uncial) letters).

    The following is a mechanization of this inference-rule scheme, followed by
    the example on page 11 of G&S. You do not (yet) need to understand how this
    mechanization works.
...
inferenceRuleSubstitution[e_, v_:List, f_:List] := (* E, v, F from G&S *)
...
(* ****************************************************************************
    ...
    Almost all the time we want the behavior of SameQ because we want results to
    be either True or False and nothing else. Now we can apply the inference
    rule. You'll see a beautiful proof when you run this through mathics (or
    Mathematica). Notice again that mathics does a little rearranging because it
    treats "+" as commutative, rewriting j + 5 as 5 + j. It's OK for now, but
    later we won't be able to assume commutativity.
...
\__,_|_||_\__,_| |___\__, |\_,_\__,_|_|_|\__|\_, |
    ...
    useful. That's why we say we use mathics as a "proof assistant" rather than
    as a "theorem prover." We simply avoid the things we know to be risky, like
    ...
    also forces us to learn more than we need to learn about the evaluator. The
    evaluator is very complicated, tuned for more general mathematics than we
    need (see https://library.wolfram.com/infocenter/Courseware/377/).
...
(* ****************************************************************************
    ...
    The underscore in mathics is not an ordinary character, not part of the name
    "a_" for instance. It's operator syntax for "Blank[]", a pattern that can
    match any expression. Let's decode the syntax "a_" via "FullForm":

        In[2]:= FullForm[a_]
        Out[2]= Pattern[a, Blank[]]

    "FullForm" decodes any syntax in mathics: it will tell you the fundamental,
    bottom meaning of any expression. We see that "a_" is a pattern named "a"
    and able to match any expression, because that's what "Blank[]" does.
    ...
    values that appear in the target of the match, then replace it with the
    expression 'a / b', which is exposed to automatic cancellation." Writing an
    explicit rule like that allows us to control when cancellation occurs.
    ...
    Holds. Let's create inert symbols, sameq, plus, times, div, etc., replacing
    them explicitly with built-ins only when we want, with rules like the one
    for "div" above. The purpose of the experiment is to see whether losing
    pretty syntax is worth avoiding struggles with evaluation leaks and the zoo
    of Holds.

    Let's first redo the substitution rule (G&S 1.1). During this experiment, we
    avoid the zoo of Holds, and avoid SetAttributes upvalues, and more details
    of the evaluator. We used some of those details in the prior definition of
    "inferenceRuleSubstitution", but we'll write a new G&S substitution rule
    which won't use those details any more. You should spend some time
    understanding the following definition.

 *************************************************************************** *)

ClearAll[substitutionInferenceRule];
substitutionInferenceRule[e_, v_:List, f_:List] := (* E, v, F from G&S *)
...
(* ****************************************************************************
    ...
    In the printout from this "expect" in Expression 25, mathics aggressively
    rewrites "CompoundExpression[x, y, z]" as "x ; y ; z" despite the fact that
    "expect" has attribute "HoldAllComplete" and "HoldForm". This is an example
    of an evaluation leak that would be very time-consuming to fix. Mathematica
    does the same thing. We won't bother to fix it, because we're trying to make
    the following point:
    ...
    important and sometimes not. If you have some expression that does not
    produce what you expect, first make sure you have your semicolons right.
...
(* ****************************************************************************
    ...
    You should see something like this:
        ...
        {Test[27]:=
            , transitivityLaw[sameq[x + y, w + 1], sameq[w + 1, 7]],
    ...
    Remember mathics has hard-coded versions of all the laws above and we must
    not trigger them. We supply our own rewrite rules for plus, times, sameq,
    and so on, leaving those as inert symbolic expressions in lower-case until
    we want to reduce them by force; mathics doesn't know how to reduce such
    expressions until we tell it explicitly by applying rewrite rules.

        U N N A M E D   R U L E S

    We have two kinds of rewrite rules: named and unnamed. Many of the ones
    above are unnamed. Many unnamed rules have the form "pattern -> result", for
    example "x -> 2y". In that example, the pattern is "x" and must match the
    target of the rule exactly. It's a pattern constant, not a pattern variable.
    We know that it's a pattern constant because there is no underscore; it's
    not "x_". The "target"" of such a rule is the left-hand side of a "/.",
    "ReplaceAll", or the left-hand side of a "//.", "ReplaceAllRepeated"
    expression. For example, in the expression

        x + z /. {x -> 2y}

    "x + z" is the target of the rule "x -> 2y". The result of evaluating the
    entire expression is "2y + z" because the "x" pattern in the rule
    (right-hand side) exactly matches the "x" in the target (left-hand side) "x
    + z". "FullForm" explains the syntax (here we must use a HoldForm to prevent
    evaluation before FullForm gets the expression):
    ...
    necessary. If there are multiple rules to apply in parallel, there is no
    choice: they must be enclosed in curly braces:
    ...
    variables match almost anything. When the rule is applied, the pattern
    variables are replaced on the right-hand side of the arrow by the things
    they match:
    ...
    "div[a_, b_] -> a / b", mathics evaluates the right-hand side when parsing
    the rule itself, before applying it to any target. Consider the following
    expression:

        div[1764, 42] /. {div[a_, b_] -> a / b}

    In a clean environment, where "a" and "b" have no definitions, mathics
    ...
    If, however, "b" had been defined somewhere else, we don't get what we
    expect:
    ...
    That's because mathics evaluates the right-hand side "a / b", early, when
    parsing In[11]. The right-hand side becomes "a / 6". Later, at replacement
    time, mathics replaces "a_" and "b_" with "1764" and "42", as before, but,
    this time the right-hand side is a / 6; "b" no longer appears, it's been
    eaten up, and we get "1764 / 6" or "294".
    ...
    "non-local" definitions make software "rigid" and "brittle," difficult to
    ...
    what's been defined in the global environment, because mathics replaces the
    pattern variables before evaluating the right-hand side of the rule, not
    after evaluating the right-hand side, as with "Rule" or "->":
    ...
    "transitivityLaw[[sameq[x+y, blahblah]...]]". The rest of the rule works
    ...
    needed. That practice avoids any need for omniscience of the global
    ...
    One more important fact must be emphasized: when mathics applies named
    rules, mathics keeps rewriting until nothing changes any more. That is the
    normal evaluation strategy. Sometimes, we want a single application of a
    rule, and for that, we must explicitly use "ReplaceAll".
...
(* Rather than do a bunch more laws (at least eight) for associativity between
   ...
   point, assuming we humans are just smart enough to handle the finesse, but
   computers are stupid and must be told exactly what to do all the time. *)
...
(* ****************************************************************************
    ...
    Call this expression "target" so we don't have to write it out long-hand
    over and over again, remembering to ClearAll it along with other definitions
...
(* ****************************************************************************
    ...
    over and over (google "don't repeat yourself"), introduce "Postfix notation
    for application of rules." Instead of
    ...
    FullForm for it is illuminating

        In[2]:= FullForm[HoldForm[p // f [#1, e] &]]
        Out[2]= (Function[f[Slot[1], e]])[p]

    "f[#1, e]&" is a Function with body "f[#1, e]" and one input parameter,
    "Slot[1]". The "//" operator just applies that function to the actual
    argument "p".

    "//" notation chains nicely (associates to the left), as we see below.
    Chaining can make proofs look more like those in the book instead of like
    deeply nested function applications staircasing off to the right. Once
    again,

        f[#1, blahblah] &

    means "a function (or rule) with body f[#1, blahblah], with slot #1 for the
    first argument."
...
(* ****************************************************************************
    ...
    named and ad-hoc rules.
        ...
        We've just glossed over an important distinction between a "proof
        assistant" and a "theorem prover." A proof assistant relies on you to
        provide the insight: the rules to apply. The proof assistant then does
        the tedious drudgery of applying the rules, plus makes fewer mistakes
        than even the most skilled people will make with pen and paper. A
        theorem prover would do a big trial-and-error search over a catalogue of
        rules. Theorem provers can be very slow and very difficult to program,
        plus they're usually not guaranteed to terminate.
...
sameq[div[e, c ^ 2], m]
    ...
    (Technical jargon, "ReplaceAllRepeated keeps applying the rules until
    'normal form' is reached, via the 'confluence' property of conditional term
    ...
    'metacircular evaluator'. See
    https://mitpress.mit.edu/sites/default/files/sicp/full-text/book/book-Z-H-25.html#%25_chap_4.
...
| (__| ' \/ -_) _` |  _| \__ \ ' \/ -_) -_)  _| (_-</ _ \ |  _/ _` | '_|
 \___|_||_\___\__,_|\__| |___/_||_\___\___|\__| /__/\___/ |_| \__,_|_|
    ...
    (* 1.4 *) transitivityLaw [sameq[x_, y_], sameq[y_, z_] ] :=
        sameq[x, z]
...
|_| \_,_|_||_\__|\__|_\___/_||_| |___|\_/\__,_|_|\_,_\__,_|\__|_\___/_||_|
    ...
    However, in general, modifying things is "code smell" to avoid.
    ...
    These two forms, the REWRITE FORM and the LAMBDA FORM for "square," are
...
(* ****************************************************************************
    ...
    We introduce a global three-term overload for "apply", which was inert in
    the past, in terms of substitution, after definition 1.7 on page 14 of the
...
PrettyPrint. See
https://mathematica.stackexchange.com/questions/2809/programmatic-formatting-for-mathematica-code-possible. *)
